...
def fetchHistory(coin, timeframe, exchange):
    last_src = pd.DataFrame(exchange.fetch_ohlcv(coin, timeframe))
...
def fetchHistoryData(coin, timeframe, exchange):
    src = pd.DataFrame(exchange.fetch_ohlcv(coin, timeframe))
